...
class PromptEvaluator:
    ...
    def generate_unique_ideas(self,
        ...
        claude.add_user_message(rendered_prompt)
        claude.add_assistant_message("```json")
        # Get response
        text = claude.chat(system = system_prompt,
                           stop_sequences = ["```"],
                           temperature = 1.0)
    ...
    def generate_dataset(self,
        ...
        """
        Generate a dataset of test cases based on the task description and save to file.
        """
    ...
    def grade_output(
        ...
        """Grade the output of a test_case using the model"""
        prompt_inputs = ""
        ...
        * The full 1-10 scale should be utilized - don't hesitate to give low scores when warranted
        ...
        claude = ClaudeResponses()
        claude.add_user_message(eval_prompt)
        claude.add_assistant_message("```json")
        # Get response
        eval_text = claude.chat(
            stop_sequences = ["```"],
            temperature = 0.0
        )
